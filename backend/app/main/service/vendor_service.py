...
# The functino to add and update user rate
def update_rate(username, truck_name, val):
    try:
        truck = Vendor.objects.get(displayed_name=truck_name)
        rate = truck.user_rate.filter(rateId=username).first()
        if rate == None:
            truck.user_rate.create(rateId=username, rate_val=val)
            all = truck.overall_rate + val
            count = truck.rateCount + 1
            truck.update(rateCount=count, overall_rate=all, avg_rate=all / count)
        else:
            all = truck.overall_rate - rate.rate_val + val
            truck.user_rate.filter(rateId=username).update(rate_val= val)
            truck.update(overall_rate=all, avg_rate=all / truck.rateCount)
        truck.save()
        return "truck rate {} is updated".format(truck_name)
    except Vendor.DoesNotExist:
        return "contain invalid truck usernames"
